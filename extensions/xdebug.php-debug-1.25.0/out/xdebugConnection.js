...
class InitPacket {
    ...
    constructor(document, connection) {
        ...
        this.engineName = (_a = documentElement.getElementsByTagName('engine')[0].textContent) !== null && _a !== void 0 ? _a : '';
...
class Connection extends dbgp_1.DbgpConnection {
    ...
    async sendPropertyGetCommand(property) {
        return new PropertyGetResponse(await this._enqueueCommand('property_get', `-d ${property.context.stackFrame.level} -c ${property.context.id} -n ${escape(property.fullName)}`), property.context);
    }
